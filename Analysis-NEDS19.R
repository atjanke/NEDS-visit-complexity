...
tic()
neds19 <- readRDS("__data-cleaned/neds19.rds")
toc()
...
neds19 <- neds19 %>% filter.(HOSP_ED %in% list_of_sites$HOSP_ED)
...
tic()
core <- fread("../../../../Data/neds/2019_NEDS/NEDS_2019_Core.csv")
...
neds19 <- neds19 %>% filter.(DISP_ED!=2)

# Save the analysis sample
saveRDS(neds19,"__data-analysis/neds19-analysis.rds")

# Save a stratified subsample
library(splitstackshape)
neds19_strat <- stratified(neds19, c("HOSP_ED"), 0.1)
saveRDS(neds19_strat,"__data-analysis/neds19-analysis-subsample.rds")



# Save the hospital-level data
data <- fread("../../../../Data/neds/2019_NEDS/NEDS_2019_Hospital.csv")
...
hosp19 <- list_of_sites %>% left_join.(data,by="HOSP_ED")


saveRDS(hosp19,"__data-analysis/hosp19-analysis.rds")
